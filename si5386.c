...
#else
#include "si5386-registers-default.h"
#endif
	...
	else
	{
		...
		for (int i = 0; i < SI5386A_REVE_REG_CONFIG_NUM_REGS; i++)
		{
			if (pageid != ((si5386a_reve_registers[i].address >> 8) & 0xff))
			{
				...
				pageid = (si5386a_reve_registers[i].address >> 8) & 0xff;
				buf[0] = 0x01;
				buf[1] = pageid;
			...
			buf[0] = si5386a_reve_registers[i].address & 0xff;
			buf[1] = si5386a_reve_registers[i].value;
